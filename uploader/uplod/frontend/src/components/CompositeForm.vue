<template>
    <form @submit="remove" class="composite-form" v-if="selected.type==='delete'">
...
export default {
    ...
    mounted() {
        if(this.selected.type === 'update') {
            ...
            this.form.parent = this.selected.parent?this.selected.data.parent:''
        }
    },
    methods:{
        add(){
            ...
            this.$http(this.$endpoint, {
                ...
                if(response.ok) {
                    return response.json().then(() => {
                        this.$notify({
                            title:'お知らせ',
                            message: this.$createElement('p',{style:'color:#900'},this.form.name + '追加しました。'),
                            duration:2000
                        })
                        this.$emit('done')
                    })
